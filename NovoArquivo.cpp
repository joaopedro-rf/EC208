...
int carregaCache(int programCount)
{
	...
	linha = programCount & 0b0000000000001111;
	
	for(i = 0; i < 3; i++)
	{
		memoriaCache[i].dados[i] = memoriaProgram[programCount+i];
	}	
	i = 0;
	return 	memoriaCache[i].dados[i];
}
...
int validaCache(int programCounter)
{
	...
	i = programCounter & 0b0000000000001111;
	tag = memoriaProgram[programCounter] & 0b1111111111110000;
	
	if(memoriaCache[i].bitValid == true)
	{
